...
function Operation({ operation, authorization, callback }) {
    ...
    function HandleDelete() {
        ...
        const verification = window.confirm('Deseja realmente excluir este registro?');

        if (verification) {
            const promisse = axios.delete(URL, { data: id, headers: { Authorization: `Bearer ${token}` } });
    ...
    return (
        <OperationContainer>
            <Div>
                ...
                <OperationDescription onClick={HandleClick}>{description}</OperationDescription>
            </Div>
            <Div classname='direita'>
                <OperationValue color={type === 'deposit' ? '#6FC341' : '#C94111'}>{parseFloat(value).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</OperationValue>
